...
import {
    ...
    uploadTranscript, inviteMember, searchMeetings, setMemberTrelloId,
} from '@/lib/api';
...
export default function WorkspaceDetailPage() {
    ...
    // Trello member linking
    const [trelloInputs, setTrelloInputs] = useState<Record<string, string>>({});
    const [trelloSaving, setTrelloSaving] = useState<string | null>(null);
    const [trelloResults, setTrelloResults] = useState<Record<string, { type: 'ok' | 'err'; text: string }>>({});
    ...
    const handleInvite = async (e: React.FormEvent) => {
        ...
        setInviteMsg(null);
        try {
            await inviteMember(params.workspaceId, inviteEmail, inviteRole);
            setInviteMsg({ type: 'ok', text: `Added ${inviteEmail} as ${inviteRole}` });
    ...
    const handleLinkTrello = async (userId: string) => {
        const username = (trelloInputs[userId] || '').trim();
        if (!username) return;
        setTrelloSaving(userId);
        setTrelloResults(prev => ({ ...prev, [userId]: undefined as any }));
        try {
            await setMemberTrelloId(params.workspaceId, userId, username);
            setTrelloResults(prev => ({ ...prev, [userId]: { type: 'ok', text: `Linked @${username}` } }));
            const fresh = await fetchWorkspaceMembers(params.workspaceId);
            setMembers(fresh);
        } catch (err: any) {
            setTrelloResults(prev => ({ ...prev, [userId]: { type: 'err', text: err.message || 'Failed to link' } }));
        } finally {
            setTrelloSaving(null);
        }
    };
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            {showInvite && (
                <div className="mb-8 p-6 bg-white border border-slate-200 rounded-lg shadow-sm">
                    ...
                    <p className="text-xs text-slate-500 mb-6">Enter an email address and choose a role. <strong>Managers</strong> can upload transcripts and invite members. <strong>Members</strong> can view meetings and manage their own tasks.</p>
                    ...
                    <form onSubmit={handleInvite} className="flex flex-col sm:flex-row gap-3">
                        ...
                        <select
                            ...
                            className="bg-slate-50 border border-slate-200 rounded-md px-3 py-2 text-sm text-slate-700 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-all">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-4 gap-10">
                ...
                <div className="space-y-6">
                    <div className="bg-white border border-slate-200 rounded-lg p-6 shadow-sm">
                        <h2 className="text-xs font-semibold text-slate-900 uppercase tracking-widest mb-1 flex items-center gap-2">
                            <Users className="w-4 h-4 text-slate-400" /> Authorized Team
                        </h2>
                        <p className="text-[11px] text-slate-400 mb-5">Enter each member's Trello username to auto-assign cards when syncing.</p>
                        <div className="space-y-5">
                            {members.map((m: any) => (
                                <div key={m.user_id} className="group">
                                    <div className="flex items-center gap-3 mb-2">
                                        <div className="w-8 h-8 bg-slate-100 rounded flex items-center justify-center text-[10px] font-bold text-slate-600 border border-slate-200 shrink-0">
                                        ...
                                        {m.user.trello_member_id && (
                                            <span className="text-[10px] font-bold text-[#0052CC] bg-blue-50 border border-blue-100 rounded px-1.5 py-0.5 shrink-0">Trello ✓</span>
                                        )}
                                    </div>
                                    <div className="flex gap-1.5 ml-11">
                                        <input
                                            type="text"
                                            placeholder={m.user.trello_member_id ? 'Update username...' : 'trello-username'}
                                            value={trelloInputs[m.user_id] || ''}
                                            onChange={e => setTrelloInputs(prev => ({ ...prev, [m.user_id]: e.target.value }))}
                                            className="flex-1 text-xs bg-slate-50 border border-slate-200 rounded px-2.5 py-1.5 focus:outline-none focus:ring-1 focus:ring-[#0052CC] transition min-w-0"
                                        />
                                        <button
                                            onClick={() => handleLinkTrello(m.user_id)}
                                            disabled={trelloSaving === m.user_id || !trelloInputs[m.user_id]?.trim()}
                                            className="shrink-0 bg-[#0052CC] hover:bg-[#0747A6] disabled:opacity-40 text-white text-[11px] font-bold px-2.5 py-1.5 rounded transition inline-flex items-center gap-1">
                                            {trelloSaving === m.user_id ? <Loader2 className="w-3 h-3 animate-spin" /> : <LayoutList className="w-3 h-3" />}
                                            Link
                                        </button>
                                    </div>
                                    {trelloResults[m.user_id] && (
                                        <p className={cn("ml-11 mt-1 text-[11px] font-medium", trelloResults[m.user_id].type === 'ok' ? 'text-emerald-600' : 'text-rose-600')}>
                                            {trelloResults[m.user_id].text}
                                        </p>
                                    )}
                                </div>
                            ))}
